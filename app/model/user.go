...
func (u *User) statistics(cont string) (err error) {
	...
	err = db.Table("dealt_run").Where("uid = ?", id.UserID).Count(&i.Value).Error
	if err != nil {
	...
	*data = append(*data, *i)
	...
	err = db.Table("dealt_walk").Where("uid = ?", id.UserID).Count(&i.Value).Error
	if err != nil {
		config.GetLogger().Warnw("数据库错误",
			"err:", err,
		)
	...
	*data = append(*data, *i)
	...
	err = db.Table("dealt_shakehand").Where("uid = ?", id.UserID).Count(&i.Value).Error
	if err != nil {
		config.GetLogger().Warnw("数据库错误",
			"err:", err,
		)
